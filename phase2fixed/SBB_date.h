...
// calculate number of periods given starting and ending date, and frequency
class Term {
	public:
		static int getNumberOfPeriods(SBB_date from_dt, SBB_date to_dt, int f) {
			int count = 0;
			while (from_dt != to_dt) {
				to_dt.add_months(-12/f);
				count++;
			}
			return count;
		}	
