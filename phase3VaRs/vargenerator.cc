...
void
VarGenerator::getVaR(double& VaR, double& VaR_CS, double& VaR_IR)
{
	...
	for(int i=0;i<length;i++) {
		...
		double rate_CS[101];
		double rate_IR[101];
		double price = records_cal[i]->getPrice();
		double rate = records_cal[i]->getRate();
		...
		for(int j=1;j<_historical_data_size;j++) {
			...
			PnL[j-1] += (records_cal[i]->getPrice(r)-price) * records[i].Amount;
