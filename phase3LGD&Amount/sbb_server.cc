...
//		ntohs(sock_addr_from_client.sin_port));
	...
	while (ret = recv(sd_current, msg, sizeof(msg), 0) > 0) {
		...
		*/
		...
		//cout<<"Var object successful"<<endl;

		double var = varCal.getVaR();
		double	port_LGD_change =  closing_port.getTotalLGD() - opening_port.getTotalLGD();
		double		port_amount_change = closing_port.getTotalAmount() - opening_port.getTotalAmount();

		timer_obj.end_clock();
		timer_obj.end_clock(real_time,user_time,system_time);
		
		sprintf(msg,"%.3f %.3f %.3f %.3f %.3f %.3f",
			var,port_LGD_change,port_amount_change,
